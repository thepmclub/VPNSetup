...
config.vpnChanger.dir = ConfigText(default="/hdd/vpn/", fixed_size=False)
...
class VpnScreen(Screen):
    ...
    def keyOK(self):
        if self.vpnList:
           ...
           if "openvpn" in str(os.listdir("/var/run")):
               ...
               self.session.open(MessageBox, _("Could not start VPN config!" % vpn), MessageBox.TYPE_ERROR, timeout=10)
               try:
                   f = open(LOGFILE, "a")
    ...
    def keyRed(self):
        if "openvpn" in str(os.listdir("/var/run")):
            os.system("/etc/init.d/openvpn stop")
            self['vpnStatus'].setText("OpenVpn is not Running")
            self.session.open(MessageBox, _("OpenVPN has stopped!"), MessageBox.TYPE_INFO, timeout=10)
            threading.Thread(target=self.readIP).start()
            try:
                f = open(LOGFILE, "a")
            ...
            if "openvpn" in str(os.listdir("/var/run")):
                ...
                try:
                    ...
                    print "Log File Error"
                self['vpnStatus'].setText("OpenVpn is not Running")
                self.session.open(MessageBox, _("OpenVPN could not be started!"), MessageBox.TYPE_ERROR, timeout=10)
    ...
    def keyYellow(self):
        if config.vpnChanger.vpnCheck.value:
            ...
            self.session.openWithCallback(self.readAnswer, MessageBox, _('Disconnect network connection when the users connection \ no longer exists? '), MessageBox.TYPE_YESNO)
            try:
                f = open(LOGFILE, "a")
    ...
    def readIP(self):
        ...
        try:
            ...
            # IP
            ipData = re.findall('<div><h1>Ihre IP-Adresse lautet:</h1></div>.*?'
                            '<div class="title"><strong>([0-9]+.[0-9]+.[0-9]+.[0-9]+)</strong></div>', data, re.S)
            # png
																			 
            pngData = re.findall('<span>Ihre System-Informationen:</span>.*?<div class="location"><img src="(https://static.wie-ist-meine-ip.de/img/country/.*?png)"', data, re.S)
        ...
        if config.vpnChanger.vpnCheck.value and self.isClose:
           ...
           if dev and dyn:
              if os.path.exists("/sys/devices/virtual/net/" + dev):
                ...
                try:
                    message = self.session.openWithCallback(self.stopCheck, MessageBox, _("There is no vpn connection! \ No dev tunnel found!"),
                                  MessageBox.TYPE_ERROR, timeout=5)
                    self.isClose = False
                except:
                    print "Session Error"
              ...
              try:
                  message = self.session.openWithCallback(self.stopCheck, MessageBox, _("There is no vpn connection! \ No config found!"),
                              MessageBox.TYPE_ERROR, timeout=5)
                  self.isClose = False
              except:
                  print "Session Error"
